...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        ...
        while(expressions.rest is not nil):
            eval_value = scheme_eval(expressions.first, env)
            value = is_scheme_false(eval_value)
            if value:
                return eval_value
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        ...
        while(expressions.rest is not nil):
            eval_value = scheme_eval(expressions.first, env)
            value = is_scheme_true(eval_value)
            if value:
                return eval_value
            expressions = expressions.rest
        return scheme_eval(expressions.first, env)
...
def make_let_frame(bindings, env):
    ...
    while(bindings is not nil):
        ...
        validate_form(binding,2,2) #check that binding has the length of exactly 2
        formal_parameter = binding.first
        binding_value = scheme_eval(binding.rest.first, env)
        # print("DEBUG : Formal Parameter", formal_parameter)
        # print("DEBUG : Value", binding_value)
        names = Pair(formal_parameter, names)
        validate_formals(names)
        values = Pair(binding_value, values)
        # validate_formals(names)
